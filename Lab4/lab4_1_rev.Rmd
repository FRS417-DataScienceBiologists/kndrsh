...
```
```{r}
readr::read_csv("/Users/clmuser/Desktop/class_files-master/Gaeta_etal_CLC_data.csv")
```
...
msleep %>%
group_by(order) %>%
  summarize (mean_nrain_wt=mean(brainwt))
```
...
msleep %>%
group_by(order) %>%
  summarize (mean_nrain_wt=mean(brainwt, na.rm =TRUE))
```
